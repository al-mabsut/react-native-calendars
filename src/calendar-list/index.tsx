...
export interface CalendarListProps extends CalendarProps, Omit<FlatListProps<any>, 'data' | 'renderItem'> {
  ...
  /** Wheather to show item separator to separate calendar cards */
  ItemSeparatorComponentCalendarList?: React.ComponentType<any> | null | undefined;
  /**
   * (ItemLayoutOffset) Used to center the current month within the view.
   * By default, this offset is set to 0. It typically equals the combined height
...
const CalendarList = (props: CalendarListProps & ContextProp, ref: any) => {
  ...
  const {
    ...
    itemLayoutOffset = 0,
    ItemSeparatorComponentCalendarList
  } = props;
  ...
  return (
    <View style={style.current.flatListContainer} testID={testID}>
      <FlatList
        ...
        ItemSeparatorComponent={ItemSeparatorComponentCalendarList}
      />
      {renderStaticHeader()}
    </View>
  );
