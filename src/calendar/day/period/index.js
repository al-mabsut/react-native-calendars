...
import { TouchableWithoutFeedback, TouchableOpacity, Text, View, Image } from 'react-native';
import { xdateToData } from '../../../interface';
import Marking, { InProgressImagePositions } from '../marking';
...
const PeriodDay = (props) => {
    ...
    const applyBorderToSection = useCallback((section, backgroundColor, borderColor, borderWidth, borderRadius, borders) => {
        if (backgroundColor) {
    ...
    const applyFullyClosedBorder = useCallback((section, backgroundColor, borderColor, borderWidth, borderRadius) => {
    ...
    const handleTwoSectionMultiPeriod = useCallback((leftFillerStyle, rightFillerStyle, marking, borderWidth, borderRadius) => {
        // Left Section
    ...
    const handleThreeSectionMultiPeriod = useCallback((leftFillerStyle, middleFillerStyle, rightFillerStyle, marking, borderWidth, borderRadius) => {
    ...
    const handleFallbackMultiPeriod = useCallback((leftFillerStyle, middleFillerStyle, rightFillerStyle, marking, borderWidth, borderRadius, hasLeft, hasMiddle, hasRight) => {
        if (hasLeft) {
    ...
    const handleSinglePeriod = useCallback((leftFillerStyle, rightFillerStyle, fillerStyle, markingStyle, borderWidth, borderRadius) => {
        const start = markingStyle.startingDay;
        const end = markingStyle.endingDay;
        if (start && !end) {
    ...
    const threeSectionFillerStyles = useMemo(() => {
    ...
    const renderFillers = useCallback(() => {
        ...
        if (marking.isMultiPeriod) {
            ...
            if (hasLeft && hasRight && !hasMiddle) {
                ...
                return (<View style={[style.current.fillers]}>
            ...
            return (<View style={[style.current.fillers, threeSectionFillerStyles.fillerStyle]}>
          <View style={[style.current.leftFiller, threeSectionFillerStyles.leftFillerStyle]}/>
          <View style={[style.current.rightFiller, threeSectionFillerStyles.rightFillerStyle]}/>
        </View>);
        }
    ...
    const renderMarking = useCallback(() => {
        ...
        return (<Marking type={'dot'} theme={theme} marked={marked} disabled={isDisabled} inactive={isInactive} today={isToday} dotColor={dotColor} dischargeIcon={marking?.dischargeIcon}/>);
    ...
    return (<Component testID={testID} disabled={touchDisabled} onPress={!touchDisabled ? _onPress : undefined} onLongPress={!touchDisabled ? _onLongPress : undefined} accessible accessibilityRole={isDisabled ? undefined : 'button'} accessibilityLabel={accessibilityLabel}>
      <View style={style.current.container}>
        {marking?.inProgressImagePosition && (<Image source={yellowStripe} style={[
                // eslint-disable-next-line react-native/no-inline-styles
                {
                    width: marking?.inProgressImagePosition === InProgressImagePositions.full
                        ? '100%'
                        : [
                    ...
                    left: marking?.inProgressImagePosition === InProgressImagePositions.right
                        ? '66%'
        ...
        <View style={containerStyle}>      
          {marking && marking.selected ? (<View style={{
                flex: 1,
